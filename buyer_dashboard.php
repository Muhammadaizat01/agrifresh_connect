...
$pageTitle = "Buyer Dashboard | AgriFresh Connect";
$activePage = 'buyer';

// Require login
if (!isLoggedIn()) {
    header("Location: login.php?redirect=buyer_dashboard.php");
    exit;
}
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && ($_POST['action'] ?? '') === 'update_profile') {
    ...
    $avatarRow = $pdo->prepare("SELECT avatar FROM users WHERE id = ?");
    $avatarRow->execute([$userId]);
    $avatarPath = $avatarRow->fetchColumn() ?: '';
    ...
    if (isset($_FILES['avatar_photo']) && $_FILES['avatar_photo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExt, $allowedExts)) {
            ...
            $err = "Invalid file type. Only JPG, PNG, WEBP, or GIF allowed.";
        }
    }

    if (!$err) {
        $upd = $pdo->prepare("UPDATE users SET name=?, email=?, phone=?, address=?, avatar=? WHERE id=?");
        $upd->execute([$name, $email, $phone, $address, $avatarPath, $userId]);
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && ($_POST['action'] ?? '') === 'update_password') {
    ...
    $passRow = $pdo->prepare("SELECT password FROM users WHERE id=?");
    $passRow->execute([$userId]);
    $hash = $passRow->fetchColumn();

    if (!password_verify($cur, $hash) && $cur !== $hash) {
...
$userRow = $pdo->prepare("SELECT name, email, phone, address, avatar FROM users WHERE id=?");
$userRow->execute([$userId]);
$profile = $userRow->fetch();
...
$displayAvatar = $userAvatar ?: 'https://ui-avatars.com/api/?name=' . urlencode($userName) . '&background=1d4ed8&color=fff&size=150';

$ordersStmt = $pdo->prepare("SELECT * FROM orders WHERE user_id=? OR buyer_name=? ORDER BY created_at DESC");
...
<div class="container-custom" style="padding: 30px 1.25rem 60px;">
    ...
    <div style="background: #eff6ff; border: 1px solid #bfdbfe; border-radius: 28px; padding: 24px; margin-bottom: 30px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 20px;">
        <div style="display: flex; align-items: center; gap: 16px;">
            <div style="position: relative; cursor: pointer;" onclick="openProfileModal()" title="Click to change profile photo">
                <img src="<?= htmlspecialchars($displayAvatar) ?>" alt="<?= htmlspecialchars($userName) ?>" style="width: 70px; height: 70px; border-radius: 22px; object-fit: cover; border: 3px solid #1d4ed8; box-shadow: 0 4px 10px rgba(29,78,216,0.2);">
            ...
            <div>
                ...
                <h1 style="font-size: 22px; font-weight: 800; color: #111827; margin-top: 4px;">👋 Hi, <?= htmlspecialchars($userName) ?>!</h1>
                <p style="font-size: 12px; color: #4b5563;">
                    <?= htmlspecialchars($userEmail) ?><?= $userPhone ? ' • ' . htmlspecialchars($userPhone) : '' ?>
                </p>
                <?php if ($userAddress): ?>
    ...
    <div style="background: #ffffff; padding: 28px; border-radius: 28px; border: 1px solid var(--border-subtle); box-shadow: var(--shadow-sm);">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; flex-wrap: wrap; gap: 10px;">
            ...
            <a href="index.php#store" style="font-size: 12px; color: #1d4ed8; font-weight: 700;">Browse Fresh Produce &rarr;</a>
        </div>
        ...
        <?php if (empty($orders)): ?>
            <div style="text-align:center;padding:40px 0;color:#6b7280;">
                ...
                <a href="index.php" style="color:#1d4ed8;font-weight:700;">Browse Fresh Produce →</a>
            </div>
        <?php else: ?>
            ...
            <div style="padding: 18px 20px; background: #f9fafb; border-radius: 18px; border: 1px solid #f3f4f6;">
                ...
                <div style="font-size:11px;color:#6b7280;margin-top:10px;display:flex;flex-wrap:wrap;gap:8px;">
                    ...
                    <?php if (!empty($order['driver'])): ?><span> | 🚚 <?= htmlspecialchars($order['driver']) ?></span><?php endif; ?>
                    <?php if (!empty($order['payment_method'])): ?><span> | 💳 <?= htmlspecialchars($order['payment_method']) ?></span><?php endif; ?>
                </div>
            </div>
            <?php endforeach; ?>
            </div>
...
<div id="profileModalBackdrop" class="modal-backdrop" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.6); backdrop-filter: blur(4px); z-index: 1000; align-items: center; justify-content: center; padding: 20px;">
    <div style="background: #ffffff; width: 100%; max-width: 520px; border-radius: 28px; padding: 28px; box-shadow: 0 20px 40px rgba(0,0,0,0.2); max-height: 90vh; overflow-y: auto;">
        ...
        <form method="POST" id="profileForm" action="buyer_dashboard.php" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 14px; font-size: 12px;">
            <input type="hidden" name="action" value="update_profile">

            <div style="text-align: center; padding: 16px; background: #f9fafb; border-radius: 20px; border: 1px solid #e5e7eb;">
                <img id="avatarModalPreview" src="<?= htmlspecialchars($displayAvatar) ?>" alt="Avatar Preview" style="width: 84px; height: 84px; border-radius: 50%; object-fit: cover; border: 3px solid #1d4ed8; margin: 0 auto 10px; display: block; box-shadow: 0 4px 10px rgba(0,0,0,0.1);">
            ...
            <div>
                ...
                <input type="text" name="name" value="<?= htmlspecialchars($userName) ?>" required class="form-input" style="width: 100%;">
            </div>

            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                <div>
                    <label style="font-weight: 700; display: block; margin-bottom: 4px;">Email Address</label>
                    <input type="email" name="email" value="<?= htmlspecialchars($userEmail) ?>" required class="form-input" style="width: 100%;">
                </div>
                <div>
                    <label style="font-weight: 700; display: block; margin-bottom: 4px;">Phone Number</label>
                    <input type="text" name="phone" value="<?= htmlspecialchars($userPhone) ?>" class="form-input" style="width: 100%;" placeholder="+60 12-345 6789">
                </div>
            </div>

            <div>
                <label style="font-weight: 700; display: block; margin-bottom: 4px;">Delivery Address</label>
                <textarea name="address" rows="2" class="form-input" style="width: 100%;"><?= htmlspecialchars($userAddress) ?></textarea>
            </div>
...
<div id="passwordModalBackdrop" class="modal-backdrop" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.6); backdrop-filter: blur(4px); z-index: 1000; align-items: center; justify-content: center; padding: 20px;">
    <div style="background: #ffffff; width: 100%; max-width: 460px; border-radius: 28px; padding: 28px; box-shadow: 0 20px 40px rgba(0,0,0,0.2); max-height: 90vh; overflow-y: auto;">
        ...
        <form method="POST" action="buyer_dashboard.php" style="display: flex; flex-direction: column; gap: 14px; font-size: 12px;">
            ...
            <div>
                ...
                <input type="password" name="confirm_password" required class="form-input" style="width: 100%;" placeholder="Repeat new password">
            </div>
